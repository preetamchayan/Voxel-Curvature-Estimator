...
__device__ void getPixelDSS2D(int2 p1, int2 p2, int2 val, int3 flag, int2* points, int *numPoints) {
    int p = (p2.y - p1.y);
    int q = (p1.x - p2.x);
...
__device__ void DSS3D(int3 p1, int3 p2, int3 minBound, int3 dim, unsigned char* voxels) {
    int3 absPoint;
...
__device__ void digitalTriangle3D(int3 p1, int3 p2, int3 p3, unsigned char* voxels, int3 dim, int3 minBound) {
